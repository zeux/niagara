...
const uint32_t kSceneCacheVersion = 7;
...
bool saveSceneCache(const char* path, const Geometry& geometry, const std::vector<Material>& materials, const std::vector<MeshDraw>& draws, std::vector<Light>& lights, const std::vector<std::string>& texturePaths, const std::vector<Animation>& animations, const std::vector<Keyframe>& keyframes, const Camera& camera, const vec3& sunDirection, uint64_t hashMeta, bool clrtMode, bool compressed, bool verbose)
{
	...
	header.keyframeCount = keyframes.size();
	...
	header.ommStates = geometry.ommStates;
	...
	fwrite(animations.data(), sizeof(Animation), animations.size(), file);
	fwrite(keyframes.data(), sizeof(Keyframe), keyframes.size(), file);

	fwrite(geometry.ommData.data(), 1, geometry.ommData.size(), file);
	fwrite(geometry.ommIndices.data(), 1, geometry.ommIndices.size(), file);
	fwrite(geometry.ommDescs.data(), sizeof(uint32_t), geometry.ommDescs.size(), file);
	...
	if (verbose)
	{
		...
		printf("OMM array data: %.2f MB\n", double(geometry.ommData.size()) / 1e6);
		printf("OMM index data: %.2f MB\n", double(geometry.ommIndices.size()) / 1e6);
	}
...
bool loadSceneCache(const char* path, Geometry& geometry, std::vector<Material>& materials, std::vector<MeshDraw>& draws, std::vector<Light>& lights, std::vector<std::string>& texturePaths, std::vector<Animation>& animations, std::vector<Keyframe>& keyframes, Camera& camera, vec3& sunDirection, uint64_t hashMeta, bool clrtMode, int ommStates)
{
	...
	geometry.ommStates = header.ommStates;
	geometry.ommData.resize(header.ommArrayDataSize);
	geometry.ommIndices.resize(header.ommIndexDataSize);
	geometry.ommDescs.resize(header.ommDescCount);
	...
	animations.resize(header.animationCount);
	keyframes.resize(header.keyframeCount);
	...
	read(animations.data(), sizeof(Animation), animations.size(), file, fileOffset);
	read(keyframes.data(), sizeof(Keyframe), keyframes.size(), file, fileOffset);
